...
export function CustomerFrequencyChart({ data }: Props) {
  return (
    <div className="bg-card rounded-2xl border border-border/60 shadow-card p-5">
      <h3 className="text-sm font-semibold font-display text-foreground mb-4">Customer Order Frequency</h3>
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
          ...
          <Tooltip formatter={(value: number) => [`${value} customers`, "Count"]} contentStyle={{ borderRadius: "12px", fontSize: 12 }} />
