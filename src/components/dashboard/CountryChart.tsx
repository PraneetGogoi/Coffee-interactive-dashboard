...
interface Props {
  data: { name: string; value: number; orders: number; customers: number }[];
}

export function CountryChart({ data }: Props) {
  return (
    <div className="bg-card rounded-2xl border border-border/60 shadow-card p-5">
      <h3 className="text-sm font-semibold font-display text-foreground mb-4">Revenue by Country</h3>
      <ResponsiveContainer width="100%" height={240}>
        <BarChart data={data} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
          ...
          <Tooltip
            formatter={(value: number, name: string) => [`$${value.toFixed(0)}`, name === "value" ? "Revenue" : name]}
            contentStyle={{ borderRadius: "12px", fontSize: 12 }}
          />
          <Bar dataKey="value" fill="hsl(250, 65%, 55%)" radius={[8, 8, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
